...
/**
 * Modal penyelesaian pekerjaan jendela maintenance dengan upload BANYAK foto.
 * - Pilih banyak file sekaligus atau jepret dari kamera (auto-unggah).
 * - Galeri thumbnail + hapus per foto + lightbox.
 * - Tombol "Selesai" aktif hanya bila ada minimal 1 foto → baru status jadi "selesai".
 */
export default function MaintenanceWindowCompleteModal({ item, onClose, onCompleted }: {
  item: MaintenanceWindow;
  onClose: () => void;
  onCompleted: () => void;
}) {
  const [photos, setPhotos] = useState<MWPhoto[]>([]);
  ...
  async function load() {
    setLoading(true);
    try { const r = await api.get(`/maintenance-windows/${item.id}/photos`); setPhotos(r.data.photos); }
  ...
  async function removePhoto(id: number) {
    ...
    try { await api.delete(`/maintenance-windows/photos/${id}`); } catch { load(); }
  }
  ...
  async function complete() {
    ...
    setBusy(true); setErr('');
    try {
      await api.put(`/maintenance-windows/${item.id}/complete`, { note: note.trim() || undefined });
      onCompleted();
    } catch (e: any) {
      setErr(e?.response?.data?.error || 'Gagal menyelesaikan pekerjaan.');
      setBusy(false);
    }
  }
    ...
    <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-surface border border-border rounded-xl w-full max-w-lg p-5 max-h-[92vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-start justify-between mb-1">
          <h3 className="text-sm font-bold">{done ? '📸 Dokumentasi Pekerjaan' : '✅ Selesaikan Pekerjaan'}</h3>
          <button onClick={onClose} className="text-text2 hover:text-text text-lg leading-none">×</button>
        </div>
        <div className="text-[11px] text-text2 mb-3">{item.title}
          {done && <span className="ml-2 text-success font-semibold">· sudah selesai{item.done_by_name ? ` oleh ${item.done_by_name}` : ''}</span>}
        </div>

        {/* Zona unggah */}
        <div className="border border-dashed border-border rounded-lg p-3 mb-3 bg-surface2/40">
          <div className="text-[11px] text-text2 mb-2">📷 Unggah dokumentasi pekerjaan — bisa pilih <b>banyak foto sekaligus</b> atau jepret langsung dari kamera.</div>
        ...
        {!done && (
          <>
            ...
            <input className="w-full bg-surface2 border border-border rounded-md px-3 py-2 text-xs mb-3" value={note} onChange={(e) => setNote(e.target.value)} placeholder="Kondisi setelah pekerjaan…" />
          </>
        )}

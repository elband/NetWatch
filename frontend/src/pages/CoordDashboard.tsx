...
type Tab = 'ops' | 'performa' | 'analitik';
const TABS: { k: Tab; label: string; icon: string }[] = [
  { k: 'ops', label: 'Operasional', icon: '⚡' },
  { k: 'performa', label: 'Performa', icon: '📊' },
  { k: 'analitik', label: 'Analitik', icon: '📈' },
];

// Angka beranimasi (count-up) untuk KPI hero — halus saat mount & saat nilai berubah live.
...
export default function CoordDashboard() {
  ...
  const pendingAppr = activities.filter((a) => a.status === 'menunggu').length;
  const needAttention = (s?.unclaimed ?? 0) + pendingAppr;

  // KPI hero — menggantikan kartu 5-stat lama (hilangkan redundansi).
  const kpis: { icon: string; label: string; value: number; suffix?: string; color: string }[] = [
    { icon: '🎫', label: 'Insiden Aktif', value: s?.totalActive ?? 0, color: 'text-text' },
    { icon: '📥', label: 'Belum Diambil', value: s?.unclaimed ?? 0, color: (s?.unclaimed ?? 0) > 0 ? 'text-accent2' : 'text-text2' },
    { icon: '⏰', label: `Lewat ${coordSla}m`, value: s?.breaching ?? 0, color: (s?.breaching ?? 0) > 0 ? 'text-danger' : 'text-text2' },
    { icon: '🔧', label: 'Sedang Proses', value: s?.inProgress ?? 0, color: 'text-warn' },
    { icon: '✅', label: 'Selesai Hari Ini', value: s?.doneToday ?? 0, color: 'text-success' },
    { icon: '📈', label: 'SLA Bulan Ini', value: stats?.totals.avgSla ?? 100, suffix: '%', color: 'text-accent2' },
  ];
  ...
  return (
    <div className="space-y-4 nw-stagger">
      ...
      <div className="rounded-2xl border border-accent/20 bg-gradient-to-br from-accent/10 via-surface to-accent2/10 p-5">
        <div className="flex items-center justify-between gap-3 mb-4 flex-wrap">
          <div>
            ...
            <div className="text-[11px] text-text2 mt-0.5">Ringkasan operasional · {monthLabel}</div>
          </div>
          <div className="flex items-center gap-2">
            <Link to="/incidents" className="border border-accent/40 text-accent rounded-md px-2.5 py-1 text-[11px] font-semibold hover:bg-accent/10">🎫 Kelola Insiden</Link>
            <Link to="/performa" className="border border-accent2/40 text-accent2 rounded-md px-2.5 py-1 text-[11px] font-semibold hover:bg-accent2/10">🏆 Performa</Link>
          </div>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {kpis.map((k) => (
            <div key={k.label} className="rounded-xl bg-surface/70 border border-border px-3.5 py-3 hover:-translate-y-0.5 transition-all">
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-[9px] uppercase tracking-wide text-text2 font-semibold truncate">{k.label}</span>
                <span className="text-sm flex-shrink-0">{k.icon}</span>
              </div>
              <div className={`text-2xl font-extrabold leading-none ${k.color}`}><CountUp value={k.value} />{k.suffix}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex gap-1 bg-surface2 border border-border rounded-xl p-1 w-fit">
        {TABS.map((t) => {
          const badge = t.k === 'ops' ? needAttention : 0;
          const activeTab = tab === t.k;
          return (
            <button
              key={t.k}
              onClick={() => setTab(t.k)}
              className={`px-4 py-1.5 rounded-lg text-xs font-semibold transition-all flex items-center gap-1.5 ${activeTab ? 'bg-accent text-bg shadow-sm' : 'text-text2 hover:text-text'}`}
            >
              <span>{t.icon} {t.label}</span>
              {badge > 0 && <span className={`text-[9px] font-bold rounded-full px-1.5 py-0.5 ${activeTab ? 'bg-bg/25 text-bg' : 'bg-danger/15 text-danger'}`}>{badge}</span>}
            </button>
          );
        })}
      </div>
      ...
      {tab === 'ops' && (
        <div className="space-y-4">
          {/* Persetujuan kegiatan teknisi */}
          <Panel title={`PERSETUJUAN KEGIATAN${pendingAppr ? ` · ${pendingAppr} menunggu` : ''}`}>
      ...
      {tab === 'performa' && (
        <div className="space-y-4">
          ...
          {/* Skor Performa Persen — komponen manajerial koordinator */}
          {isScored && myScore && (
          ...
          <Panel title="🏆 PERFORMA TEKNISI & KOORDINATOR" right={<Link to="/performa" className="text-[11px] text-text2 hover:text-text">Detail →</Link>}>
            {skorList.length === 0 ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 nw-stagger">
                {skorList.map((pr) => (
                  <button key={pr.userId} onClick={() => setExplainRow(pr)} className="nw-card bg-surface2 border border-border rounded-lg p-3 text-center hover:border-accent/50 transition-colors">
                    <div className="text-2xl mb-1">{pr.emoji || (pr.role === 'koordinator' ? '👔' : '🔧')}</div>
                    <div className="text-xs font-semibold">{pr.name.split(' ')[0]}</div>
                    <div className="text-[10px] text-text2 mb-2">{pr.jabatan}</div>
                    <div className={`text-xl font-bold ${gradeCls(pr.score)}`}>{pr.score == null ? '–' : pr.score}</div>
                    <div className="text-[9px] text-text2">{pr.score == null ? 'Belum dinilai' : pr.grade}</div>
                  </button>
                ))}
              </div>
            )}
      ...
      {/* Status bar bawah — selalu tampil */}
      ...
      {/* ===== MODALS (root — tampil apa pun tab-nya) ===== */}
      {showExplain && myScore && (
              ...
              <div className="px-5 py-4 overflow-y-auto flex-1 space-y-4">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {tiers.map((t) => {
                      const activeT = scoreMeta(score).label === t.label;
                      return (
                        <div key={t.label} className={`rounded-lg border px-3 py-2 ${activeT ? 'border-accent bg-accent/10' : 'border-border bg-surface2/40'}`}>
                          <div className={`text-[12px] font-bold ${t.color}`}>{t.label} {activeT && '← Anda'}</div>

...
router.get('/', requireRole('admin', 'koordinator'), async (req, res) => {
  const [rows] = await pool.query(
    `SELECT r.*, (SELECT COUNT(*) FROM public_reports p WHERE p.room_id=r.id) AS total_laporan,
       ...
       FROM rooms r ORDER BY r.gedung, r.nama`);
  res.json({ rooms: rows });
});
...
router.post('/', requireRole('admin', 'koordinator'), async (req, res) => {
  ...
  const [r] = await pool.query('INSERT INTO rooms (kode, nama, gedung, lantai, area, penanggung_jawab, active) VALUES (?,?,?,?,?,?,1)',
    [kode, b.nama.trim(), b.gedung || null, b.lantai || null, b.area || null, b.penanggung_jawab || null]);
...
router.put('/:id', requireRole('admin', 'koordinator'), async (req, res) => {
  ...
  if (!rows[0]) return res.status(404).json({ error: 'Ruangan tidak ditemukan' });
...
router.delete('/:id', requireRole('admin', 'koordinator'), async (req, res) => {
  await pool.query('DELETE FROM rooms WHERE id=?', [Number(req.params.id)]);
  res.json({ ok: true });
});
...
router.post('/bulk', requireRole('admin', 'koordinator'), async (req, res) => {
  ...
  for (const b of list) {
    ...
    try { await pool.query('INSERT INTO rooms (kode, nama, gedung, lantai, area) VALUES (?,?,?,?,?)', [kode, b.nama.trim(), b.gedung || null, b.lantai || null, b.area || null]); created++; } catch { /* lewati duplikat */ }
  }
  res.json({ created });
});
...
router.get('/stats', requireRole('admin', 'koordinator'), async (req, res) => {
  const [[s]] = await pool.query(
    `SELECT SUM(DATE(created_at)=CURDATE()) hari_ini, SUM(DATE_FORMAT(created_at,'%Y-%m')=DATE_FORMAT(CURDATE(),'%Y-%m')) bulan_ini,
            SUM(status='menunggu') menunggu, SUM(status='diproses') diproses, SUM(status='selesai') selesai FROM public_reports`);
  const [[inc]] = await pool.query(
    `SELECT AVG(duration_min) mttr, SUM(TIMESTAMPDIFF(MINUTE,created_at,taken_at)<=30) ot, COUNT(taken_at) tot
       FROM incidents WHERE source='public_report' AND created_at>=DATE_SUB(CURDATE(), INTERVAL 30 DAY)`);
  const [topLokasi] = await pool.query("SELECT COALESCE(ruang,'Tanpa lokasi') lokasi, COUNT(*) jumlah FROM public_reports GROUP BY ruang ORDER BY jumlah DESC LIMIT 5");
  const [topKategori] = await pool.query('SELECT jenis kategori, COUNT(*) jumlah FROM public_reports GROUP BY jenis ORDER BY jumlah DESC LIMIT 5');

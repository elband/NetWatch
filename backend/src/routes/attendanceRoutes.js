...
router.post('/check-in', async (req, res) => {
  ...
  // Gate: absensi dibuka 1 jam sebelum jam dinas (per unit). Tidak terjadwal = tidak digating.
  const gate = await shiftOpenGate(pool, req.user.id);
  if (gate.hasShift && !gate.allowed) {
    return res.status(400).json({ error: `Absensi masuk dibuka pukul ${gate.opensAt} (1 jam sebelum jam dinas Anda).` });
  }
